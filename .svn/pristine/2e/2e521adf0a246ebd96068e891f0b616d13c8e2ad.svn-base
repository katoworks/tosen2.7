            ...
            if(this.companyInfo != null)
            {
                ...
                for(int mCnt = 0; mCnt < famiDat.Count; mCnt++)
                {
                    ...
                    if (famiDat[mCnt].conpnyFlg == true)
                    {
                        ...
                        mDat.famiNo = mCnt;
                        mDat.comName = this.comInfo.comName;
                        mDat.comAddress = Chg(this.comInfo.comAddr);
                        mDat.tel = this.comInfo.comTel;
                        string mTmpZip = this.companyInfo.companyInfo.comZip.Replace("-", "");
                        mDat.comZip1 = mTmpZip.Substring(0, 3);
                        mDat.comZip2 = mTmpZip.Substring(3, 4);
